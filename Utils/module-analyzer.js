...
/**
 * Analyze a single module file with enhanced error recovery
...
function analyzeModuleFile(filePath, filename) {
    ...
    try {
        ...
        if (!filePath || !filename) {
            throw new Error('Invalid file path or filename provided');
        }
        ...
        if (!content || content.trim().length === 0) {
            console.warn(`   ⚠️  Warning: ${filename} appears to be empty`);
            return createEmptyModuleAnalysis(filename, 'Empty file content');
        }

        // Check for reasonable file size (warn if > 1MB)
        if (stats.size > 1024 * 1024) {
            console.warn(`   ⚠️  Warning: ${filename} is very large (${Math.round(stats.size / 1024)}KB)`);
        }

        let metadata, functions, registration, qualityAnalysis, healthScore;

        // Extract basic metadata with error recovery
        try {
            metadata = extractModuleMetadata(content, filename, stats);
        } catch (error) {
            console.warn(`   ⚠️  Metadata extraction failed for ${filename}: ${error.message}`);
            metadata = createFallbackMetadata(filename, stats);
        }

        // Parse functions with error recovery
        try {
            functions = extractFunctions(content, filename);

            if (functions.total_count === 0) {
                console.warn(`   ⚠️  No functions found in ${filename}`);
            }
        } catch (error) {
            console.warn(`   ⚠️  Function extraction failed for ${filename}: ${error.message}`);
            functions = createFallbackFunctions();
        }

        // Check registration compliance with error recovery
        try {
            registration = checkRegistrationCompliance(content, functions);
        } catch (error) {
            console.warn(`   ⚠️  Registration check failed for ${filename}: ${error.message}`);
            registration = createFallbackRegistration();
        }

        // Apply quality rules with error recovery
        try {
            qualityAnalysis = applyQualityRules(content, functions, metadata);
        } catch (error) {
            console.warn(`   ⚠️  Quality analysis failed for ${filename}: ${error.message}`);
            qualityAnalysis = createFallbackQualityAnalysis(error.message);
        }

        // Calculate health score with error recovery
        try {
            healthScore = calculateHealthScore(qualityAnalysis);
        } catch (error) {
            console.warn(`   ⚠️  Health score calculation failed for ${filename}: ${error.message}`);
            healthScore = createFallbackHealthScore();
        }
        ...
        const analysisTime = Date.now() - startTime;
        console.error(`   ❌ Analysis error for ${filename}: ${error.message}`);
        ...
        return {
            ...
            metadata: createFallbackMetadata(filename, null),
            functions: createFallbackFunctions(),
            registration: createFallbackRegistration(),
            quality: createFallbackQualityAnalysis(error.message),
            health: createFallbackHealthScore()
        };
    }
}

/**
 * Create empty module analysis for edge cases
 * @param {string} filename - File name
 * @param {string} reason - Reason for empty analysis
 * @returns {Object} Empty analysis object
 */
function createEmptyModuleAnalysis(filename, reason) {
    return {
        filename: filename,
        success: false,
        error: reason,
        ...
        functions: createFallbackFunctions(),
        registration: createFallbackRegistration(),
        quality: createFallbackQualityAnalysis(reason),
        health: createFallbackHealthScore()
    };
}
...
function createFallbackMetadata(filename, stats) {
    ...
    return {
        ...
        comment_lines: 0,
        blank_lines: 0,
        has_configuration_objects: false,
...
/**
 * Create fallback functions object when extraction fails
 * @returns {Object} Fallback functions object
 */
function createFallbackFunctions() {
    return {
        ...
        statistics: {
            ...
            longest_function: { name: 'none', line_count: 0 },
...
/**
 * Create fallback registration object when check fails
 * @returns {Object} Fallback registration object
 */
...
/**
 * Create fallback quality analysis when analysis fails
 * @param {string} errorMessage - Error message
 * @returns {Object} Fallback quality analysis
 */
function createFallbackQualityAnalysis(errorMessage) {
    return {
        es3_compliance: { compliant: false, violations: [], total_penalty: 0, critical_violations: [] },
        reserved_word_safety: { safe: false, violations: [], total_penalty: 0, critical_violations: [] },
        dependency_compliance: { has_dependency_validation: false, issues: [], total_penalty: 0 },
        logging_compliance: { modern_calls: 0, legacy_calls: 0, score: 0 },
        memory_management: { issues: [], total_penalty: 0, net_score: 0 },
        function_architecture: { score: 0 },
        api_safety: { score: 0, patterns: [] },
        code_organization: { score: 0, patterns: [], issues: [`Analysis failed: ${errorMessage}`] },
        performance_patterns: { score: 0, warnings: [], critical_issues: [] },
        documentation_quality: { score: 0, jsdoc_coverage_percentage: 0, patterns: [], issues: [] },
        configuration_validation: { score: 0, patterns: [], issues: [] },
        security_patterns: { score: 0, security_issues: [], warnings: [], risk_level: 'UNKNOWN' },
        function_similarity: { score: 0, similar_functions: [], duplicates: [] },
        overall_score: 0,
        critical_violations: []
    };
}
...
/**
 * Create fallback health score when calculation fails
 * @returns {Object} Fallback health score
 */
function createFallbackHealthScore() {
    return {
        total_score: 0,
        grade: 'F',
        critical_violations: ['Analysis failed'],
        penalties: [],
        bonuses: [],
...
/**
 * Perform cross-module analysis with enhanced error handling
...
function performCrossModuleAnalysis(moduleAnalyses) {
    ...
    try {
        ...
        if (!Array.isArray(moduleAnalyses) || moduleAnalyses.length === 0) {
            console.log(`   ⚠️  No valid module analyses provided for cross-module analysis`);
            return analysis;
        }

        const successfulModules = moduleAnalyses.filter(m =>
            m &&
            m.success &&
            m.functions &&
            m.functions.function_content &&
            typeof m.functions.function_content === 'object'
        );
        ...
        if (successfulModules.length === 1) {
            console.log(`   ℹ️  Only one module found - skipping cross-module comparison`);
            return analysis;
        }
        ...
        try {
            ...
            analysis.function_similarities = [];
        }
        ...
        try {
            ...
            analysis.dependency_violations = [];
        }
...
/**
 * Analyze function similarities across modules with performance optimizations
...
function analyzeFunctionSimilarities(modules) {
    ...
    console.log(`   🎯 Analyzing similarities with ${threshold}% threshold...`);

    // Performance optimization: early termination and progress tracking
    let comparisons = 0;
    let totalPossibleComparisons = 0;
    ...
    for (let i = 0; i < modules.length; i++) {
        ...
        for (let j = i; j < modules.length; j++) {
            ...
            totalPossibleComparisons += functionsA.length * functionsB.length;
        }
    }

    console.log(`   📊 Will perform up to ${totalPossibleComparisons} function comparisons...`);

    // Compare all functions across all modules with optimizations
    for (let i = 0; i < modules.length; i++) {
        const moduleA = modules[i];
        if (!moduleA.functions || !moduleA.functions.function_content) continue;
            ...
            functionsA.forEach(funcA => {
                functionsB.forEach(funcB => {
                    ...
                    if (comparisons % 100 === 0) {
                        const progress = Math.round((comparisons / totalPossibleComparisons) * 100);
                        console.log(`   ⏳ Progress: ${progress}% (${comparisons}/${totalPossibleComparisons})`);
                    }

                    // Skip comparing function to itself
                    if (i === j && funcA.name === funcB.name) return;

                    // Performance optimization: Quick parameter count check first
                    const paramDiff = Math.abs((funcA.parameter_count || 0) - (funcB.parameter_count || 0));
                    if (paramDiff > 2) {
                        // If parameter counts differ by more than 2, unlikely to be similar
                        return;
                    }

                    // Performance optimization: Quick name similarity check
                    if (funcA.name && funcB.name) {
                        const nameA = funcA.name.toLowerCase();
                        const nameB = funcB.name.toLowerCase();

                        // If names are completely different and don't share common patterns, skip
                        if (nameA.length > 0 && nameB.length > 0) {
                            const commonChars = getCommonCharacterRatio(nameA, nameB);
                            if (commonChars < 0.3 && !hasCommonPrefixSuffix(nameA, nameB)) {
                                return; // Names too different, likely not similar functions
                            }
                        }
                    }
    ...
    console.log(`   ✅ Completed ${comparisons} comparisons, found ${similarities.length} matches`);
...
/**
 * Calculate ratio of common characters between two strings
 * @param {string} str1 - First string
 * @param {string} str2 - Second string
 * @returns {number} Ratio of common characters (0-1)
 */
function getCommonCharacterRatio(str1, str2) {
...
/**
 * Check if two strings have common prefixes or suffixes
 * @param {string} str1 - First string
 * @param {string} str2 - Second string
 * @returns {boolean} True if they have common patterns
 */
function hasCommonPrefixSuffix(str1, str2) {
    ...
    // Check for common suffixes (last 3+ characters)
    const suffixLength = Math.min(4, minLength);
    if (str1.substring(str1.length - suffixLength) === str2.substring(str2.length - suffixLength)) {
        return true;
    }

    // Check for common patterns (get/set, create/build, etc.)
    const commonPatterns = [
        ['get', 'set'], ['create', 'build'], ['init', 'setup'],
        ['validate', 'check'], ['parse', 'process'], ['generate', 'create']
    ];

    for (const pattern of commonPatterns) {
        if ((str1.includes(pattern[0]) && str2.includes(pattern[1])) ||
            (str1.includes(pattern[1]) && str2.includes(pattern[0]))) {
            return true;
        }
    }
...
/**
 * Calculate similarity between two functions with enhanced validation
...
function calculateFunctionSimilarity(funcContentA, funcContentB, funcInfoA, funcInfoB) {
    // Enhanced input validation
    if (!funcContentA || !funcContentB || typeof funcContentA !== 'object' || typeof funcContentB !== 'object') {
        return { percentage: 0, type: 'no_content', recommendation: 'Cannot compare - invalid content objects' };
    }

    if (!funcInfoA || !funcInfoB || typeof funcInfoA !== 'object' || typeof funcInfoB !== 'object') {
        return { percentage: 0, type: 'no_info', recommendation: 'Cannot compare - invalid function info' };
    }

    // Validate required properties
    if (!funcInfoA.name || !funcInfoB.name) {
        return { percentage: 0, type: 'no_names', recommendation: 'Cannot compare - missing function names' };
    }
...
function generateFolderReportYAML(folderStats, moduleAnalyses, crossModuleAnalysis) {
    ...
    yaml.push(`  function_similarities_found: ${crossModuleAnalysis.function_similarities.length}`);
    ...
    yaml.push(`  dependency_violations: ${crossModuleAnalysis.dependency_violations.length}`);
    yaml.push(`  consolidation_opportunities: ${crossModuleAnalysis.consolidation_opportunities.length}`);
    yaml.push(`  function_duplication_score: ${folderStats.cross_module_analysis.function_duplication_score}`);
    yaml.push(`  dependency_health_score: ${folderStats.cross_module_analysis.dependency_health_score}`);
    yaml.push('');

    // TOP DUPLICATION ISSUES (if any)
    if (crossModuleAnalysis.function_similarities.length > 0) {
    ...
    // CRITICAL FIX: Actually return the YAML content!
    return yaml.join('\n');
}
